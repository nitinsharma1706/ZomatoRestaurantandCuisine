...
# Calculating the frequency of each cuisine
frq_cus = df.groupby("Cuisines").size()

# Calculating the relative frequency of each cuisine
frq_dis_cus = df.groupby("Cuisines").size() / len(df)

# Creating a copy of the DataFrame and adding a new column for cuisine frequency
df_frq_cus = df_frq_loc.copy()
df_frq_cus["loc_cus"] = df_frq_cus.Cuisines.map(frq_dis_cus)

# Dropping the "Cuisines" column from the DataFrame
